...
n8n is the backbone of this project's intelligence. It is a low-code, open-source workflow automation platform that connects APIs, AI models, and services without needing a custom backend server.
...
Icons: Font Awesome 6 — solid and brand icon library
Color Palette:
...
Log in to your n8n Cloud or self-hosted instance
Create a new workflow
Add a Webhook trigger node — set method to POST, note the webhook URL
Build your AI pipeline (see Workflow Breakdown above)
Add a Respond to Webhook node at the end, returning the HTML string
...
Zero backend maintenance — n8n handles all server-side logic, no Express, no database, no deployment pipeline
AI-agnostic — Swap the LLM in n8n any time (OpenAI → Gemini → Claude) without touching frontend code
Instant deployment — The frontend is static HTML; host it anywhere (GitHub Pages, Netlify, Vercel — free)
Highly modifiable — Change the infographic style by editing the n8n Code node, no frontend rebuild needed
Rapid iteration — Update workflow logic visually in n8n in minutes
Cost-effective — No server costs beyond n8n cloud subscription and AI API usage
📦 Tech Stack Summary
Layer	Technology
